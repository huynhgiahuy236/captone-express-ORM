...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 flex h-16 sm:h-20 items-center justify-between gap-2 sm:gap-4 bg-white dark:bg-[#181C31] px-3 sm:px-6 shadow-xs border-b border-gray-100 dark:border-[#2d2f40] transition-colors">
        {/* Left section: Logo */}
        <div className="flex items-center shrink-0">
          <HukiLogo size="md" />
        </div>

        {/* Middle section: Large Search Bar with Auto-Suggestions (Desktop/Tablet only) */}
        <div ref={searchContainerRef} className="hidden md:block relative flex-1 max-w-4xl min-w-0 mx-2 sm:mx-4">
          <form onSubmit={handleSearchSubmit} className="w-full">
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm ý tưởng, anime, ẩm thực..."
                value={searchTerm}
          ...
          {isSearchFocused && (
            <div className="fixed inset-x-2 top-16 sm:absolute sm:inset-x-0 sm:top-full mt-2 rounded-3xl bg-white dark:bg-[#1c2136] border border-gray-200 dark:border-[#2d2f40] shadow-2xl p-4 sm:p-5 z-50 animate-in fade-in slide-in-from-top-2 duration-200 max-h-[80vh] overflow-y-auto [scrollbar-width:none]">
        ...
        <div className="hidden md:flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          ...
          {!mounted ? (
            ...
            <>
              ...
              <Link
                href="/create"
                className={`flex h-9 sm:h-10 items-center gap-1.5 rounded-full px-3.5 sm:px-4 font-bold text-xs sm:text-sm transition duration-200 active:scale-95 ${
                  pathname === "/create"
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-64 sm:w-72 rounded-3xl bg-white dark:bg-[#1c2136] p-2.5 shadow-2xl ring-1 ring-black/10 dark:ring-[#2d2f40] border border-gray-100 dark:border-[#2d2f40] animate-in fade-in slide-in-from-top-2 duration-150 z-50 divide-y divide-gray-100 dark:divide-[#2d2f40]">
        ...
        {/* Mobile Hamburger Button (3 Gạch) on far right */}
        <div className="flex md:hidden items-center shrink-0">
          <button
            type="button"
            onClick={() => setMobileMenuOpen(true)}
            aria-label="Mở menu điều hướng"
            className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gray-100 dark:bg-[#1c2136] text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-[#252A42] active:scale-95 transition cursor-pointer"
          >
            <Menu size={22} />
          </button>
        </div>
      ...
      {/* ===================== MOBILE SLIDE-OVER DRAWER (MENU 3 GẠCH) ===================== */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden animate-in fade-in duration-200">
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-[85vw] max-w-sm bg-white dark:bg-[#181C31] shadow-2xl border-l border-gray-200 dark:border-[#2d2f40] flex flex-col justify-between overflow-y-auto animate-in slide-in-from-right duration-300">
            {/* Top: Header & Close button */}
            <div className="p-4 border-b border-gray-100 dark:border-[#2d2f40] flex items-center justify-between">
              <HukiLogo size="sm" />
              <button
                type="button"
                onClick={() => setMobileMenuOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-100 dark:bg-[#252A42] text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-[#2d3352] transition cursor-pointer"
                title="Đóng menu"
              >
                <X size={18} />
              </button>
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto">
              ...
              >
                ...
                <input
                  type="text"
                  placeholder="Tìm kiếm ý tưởng, tranh ảnh..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="h-10 w-full rounded-2xl bg-gray-100 dark:bg-[#252A42] pl-10 pr-4 text-xs font-semibold text-gray-800 dark:text-white placeholder-gray-400 outline-hidden border border-transparent focus:border-[#0052cc]"
                />
              </form>
              ...
              {user ? (
                <Link
                  href="/profile"
                  onClick={() => setMobileMenuOpen(false)}
                  className="p-3.5 rounded-2xl bg-blue-50/70 dark:bg-[#252A42] flex items-center gap-3 border border-blue-100 dark:border-[#2d2f40] group transition"
                >
                  <div className="h-11 w-11 overflow-hidden rounded-full border border-blue-200 dark:border-blue-900 bg-white shrink-0">
                    {user.anh_dai_dien ? (
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-extrabold text-sm text-gray-900 dark:text-white truncate group-hover:text-[#0052cc] dark:group-hover:text-blue-400">
                      {user.ho_ten || "Người dùng"}
                    </p>
                ...
                <div className="p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-[#252A42] dark:to-[#1c2136] border border-blue-100/80 dark:border-[#2d2f40] space-y-3">
                  <div>
                    <h3 className="font-black text-sm text-gray-900 dark:text-white">Chào mừng đến với HUKI!</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Khám phá và lưu giữ hàng triệu ý tưởng sáng tạo</p>
                  </div>
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <Link
                      href="/login?mode=login"
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center justify-center h-9 rounded-xl bg-[#0052cc] text-white font-bold text-xs shadow-xs active:scale-95 transition"
                    >
                      Đăng nhập
                    </Link>
                    <Link
                      ...
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center justify-center h-9 rounded-xl bg-white dark:bg-[#181C31] text-gray-900 dark:text-white border border-gray-200 dark:border-[#2d2f40] font-bold text-xs shadow-xs active:scale-95 transition"
                    >
                      Đăng ký
                    </Link>
                  </div>
              ...
              {/* Navigation Links */}
              <div className="space-y-1">
                <p className="px-2 text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-400 mb-1">
                  Điều hướng chính
                </p>
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition ${
                    pathname === "/"
                      ? "bg-blue-50 dark:bg-[#252A42] text-[#0052cc] dark:text-blue-400"
                      : "text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#252A42]"
                  }`}
                >
                  <Home size={17} />
                  <span>Trang chủ Khám phá</span>
                </Link>
                ...
                >
                  <PlusCircle size={17} className="text-[#0052cc] dark:text-blue-400" />
                  <span>Tạo Ghim / Tải ảnh lên</span>
                </Link>
                ...
                {user && (
                  <>
                    ...
                    <Link
                      href="/profile?tab=saved"
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#252A42] transition"
                    >
                      <FolderHeart size={17} />
                      <span>Folder & Ghim đã lưu</span>
                    </Link>
                    ...
                    <Link
                      ...
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#252A42] transition"
                    >
                      <Shield size={17} className="text-[#0052cc] dark:text-blue-400" />
                      <span>Cài đặt Quyền riêng tư</span>
                    </Link>
                  </>
                )}
              ...
              <div className="space-y-2">
                <p className="px-2 text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-400">
                  Danh mục nổi bật
                </p>
                <div className="grid grid-cols-2 gap-1.5">
                  {TRENDING_SUGGESTIONS.map((cat, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => {
                        setMobileMenuOpen(false);
                        handleSelectQuery(cat.query);
                      }}
                      className="flex items-center gap-2 p-2 rounded-xl bg-gray-50 dark:bg-[#252A42]/70 text-left hover:bg-blue-50 dark:hover:bg-[#2e3555] transition text-xs font-semibold text-gray-800 dark:text-gray-200"
                    >
                      <span>{cat.icon}</span>
            ...
            <div className="p-4 border-t border-gray-100 dark:border-[#2d2f40] space-y-2">
              <button
                type="button"
                onClick={toggleTheme}
                className="flex w-full items-center justify-between px-3 py-2.5 rounded-xl bg-gray-100 dark:bg-[#252A42] text-xs font-bold text-gray-800 dark:text-gray-200 transition cursor-pointer"
              >
                <div className="flex items-center gap-2.5">
                  {mounted && theme === "dark" ? <Sun size={17} className="text-yellow-400" /> : <Moon size={17} className="text-gray-600 dark:text-gray-300" />}
                  <span>Chế độ: {mounted && theme === "dark" ? "Tối (Dark)" : "Sáng (Light)"}</span>
                </div>
                <span className="text-[10px] text-[#0052cc] dark:text-blue-400 bg-white dark:bg-[#181C31] px-2 py-0.5 rounded-md shadow-2xs">
                  Chuyển
                </span>
              </button>
              ...
              {user && (
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-100 transition cursor-pointer"
                >
                  <LogOut size={16} />
